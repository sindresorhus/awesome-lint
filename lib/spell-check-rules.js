...
// Note: Trailing matches intentionally omit a trailing `\b`, so e.g. "mocha tests" matches via "test". This is a cheap way to catch plurals/variants without listing every form.
const standaloneTechRule = (word, value, {leading = [], trailing = []} = {}) => ({
	test: [
		new RegExp(`^${escapeRegex(word)}$`, 'gi'),
		...leading.map(context => new RegExp(`(?<=\\b${escapeRegex(context).replaceAll(' ', String.raw`\s+`)}\\s+)${escapeRegex(word)}\\b`, 'gi')),
		...(trailing.length === 0 ? [] : [new RegExp(`\\b${escapeRegex(word)}\\b(?=\\s+(?:${trailing.map(trailingWord => escapeRegex(trailingWord)).join('|')}))`, 'gi')]),
	],
	value,
});

const leadingTechnologyContextWords = [
	'uses',
	'using',
	'built with',
	'deploy with',
...
const spellCheckRules = [
	...
	standaloneTechRule('parcel', 'Parcel', {
		leading: leadingTechnologyContextWords,
		trailing: ['bundler', 'bundlers', 'build', 'builds', 'dev', 'plugin', 'plugins', 'config', 'configs', 'js', 'v2', 'transformer', 'transformers', 'target', 'targets', 'watcher', 'watchers'],
	}),
	{
		test: 'rollup',
		value: 'Rollup',
	...
	standaloneTechRule('babel', 'Babel', {
		leading: leadingTechnologyContextWords,
		trailing: [
			'plugin',
			'plugins',
	...
	standaloneTechRule('mocha', 'Mocha', {
		leading: leadingTechnologyContextWords,
		trailing: [
			'test',
			'tests',
	...
	standaloneTechRule('jasmine', 'Jasmine', {
		trailing: ['test', 'spec', 'framework', 'suite', 'spy', 'expect', 'runner', 'describe', 'js'],
	}),
	{
		test: 'cypress',
		value: 'Cypress',
	...
	standaloneTechRule('llama', 'LLaMA', {
		trailing: ['model', 'index', 'chat', 'api', 'inference', 'embed', 'weight', 'format', 'file', 'hub', 'pack'],
	}),
	{
		test: 'chatgpt',
		value: 'ChatGPT',
	...
	standaloneTechRule('blender', 'Blender', {
		leading: leadingTechnologyContextWords,
		trailing: [
			'python',
			'addon',
	...
	standaloneTechRule('slack', 'Slack', {
		leading: leadingTechnologyContextWords,
		trailing: [
			'api',
			'apis',
	...
	standaloneTechRule('alpine', 'Alpine', {
		trailing: ['linux', 'js', 'docker', 'container', 'image', 'package', 'apk', 'musl'],
	}),
	{
		test: 'freebsd',
		value: 'FreeBSD',
	...
	standaloneTechRule('rider', 'Rider', {
		trailing: ['ide', 'jetbrains', 'unity', 'plugin', 'extension', 'csharp', 'dotnet'],
	}),
	standaloneTechRule('eclipse', 'Eclipse', {
		trailing: ['plugin', 'ide', 'java', 'rcp', 'equinox', 'jdt', 'pde', 'marketplace', 'workspace', 'project', 'che'],
	}),
	{
		test: 'netbeans',
		value: 'NetBeans',
	...
	standaloneTechRule('mercurial', 'Mercurial', {
		trailing: ['vcs', 'repository', 'branch', 'commit', 'pull', 'push', 'workflow', 'extension', 'hook', 'plugin'],
	}),
	{
		test: 'postman',
		value: 'Postman',
	},
	standaloneTechRule('insomnia', 'Insomnia', {
		trailing: ['api', 'rest', 'client', 'plugin', 'request', 'collection', 'environment', 'test'],
	}),
	{
		test: 'wireshark',
		value: 'Wireshark',
